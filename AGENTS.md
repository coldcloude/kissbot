...
- 本文件（AGENTS.md）记录了项目整体的开发计划，其中 架构设计 章节的内容根据 开发者决策.txt 中的决策进行设计，随着开发过程不断变更，体现项目架构的最新变化
- 项目每个目录代表一个模块，每个模块目录下都使用一个独立的 AGENTS.md 记录模块的架构设计和实现约定，随着开发过程不断变更，体现模块的最新变化
