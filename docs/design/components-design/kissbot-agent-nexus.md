...
- **用户故事**：
  - 作为一个管理员，我要启动智能体程序，以让智能体恢复近期对话记忆并构建初始 LLM 上下文
  - 作为一个管理员，我要发送重置上下文命令，以让智能体重建初始 LLM 上下文
...
- **用户故事**：
  ...
  - 作为一个管理员，我要切换智能体的记忆模式（角色/事件），以让智能体按模式隔离记忆范围并重建上下文
...
- **用户故事**：
  - 作为一个管理员，我要配置通道绑定列表，以让智能体启动时连接配置的通道并绑定用户，实现与多个消息通道的通信
...
- 启动或上下文重置时，记忆读取器按当前模式从 memory-store 读取最近历史记录、从 memory-struct 读取顶层记忆索引（如摘要列表，memory-struct 未实现时跳过），并结合 memory-ego 的自我认知信息构建初始 LLM 上下文
- 运行时每次收到用户消息和 LLM 回复后增量追加到上下文，保持对话连续性
...
- 启动时读取配置中的通道绑定列表，为每个通道建立连接，连接后自动获取通道信息并发送绑定请求绑定指定用户
- 每个连接独立运行，支持心跳检测和断线自动重连
- 上行消息送入协调器处理，下行消息根据目标 messenger ID 选择对应连接发送，并接收通道返回的发送结果
