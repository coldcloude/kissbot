...
- **依赖**：无
- **用户故事**：
  - 作为一个 kissbot 服务进程，我要在启动时从指定路径加载 JSON 配置文件，以初始化运行时配置
  - 作为一个 kissbot-config 库，我要在配置加载失败时立即 panic，以在启动阶段暴露配置错误
  - 作为一个组件，我要通过点号路径导航到配置的指定层级，以获取自己需要的配置段
  - 作为一个组件，我要在指定配置段不存在或类型不匹配时得到 panic，以在启动阶段暴露配置缺失
...
- **依赖**：kissbot-config（配置加载）
- **用户故事**：
  - 作为一个 HTTP 服务组件，我要从公共配置中读取 api_key，以验证请求来源的认证信息
  - 作为一个 WS 服务组件，我要从公共配置中读取 api_key，以验证 WebSocket 握手阶段的认证信息
  - 作为一个 channel-web，我要从公共配置中读取 admin_api_key，以区分管理员和普通用户的访问权限
  - 作为一个 HTTP 客户端组件，我要从公共配置中读取 api_key，以在请求中附加认证头
