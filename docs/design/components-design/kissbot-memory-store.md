...
- 路径一（由消息通道推送）：通道实现组件接收外部消息 → 通道管理器消息队列 → 发送到记忆存储组件
- 路径二（由 nexus 推送）：nexus 产生或收到的思考/工具调用/回复/tool 结果 → 推送到记忆存储组件

推送方（nexus/channel）按记忆模式拼接标识后缀 → 将 (agent-id, year, suffix) 传递给记忆基础组件 → 记忆基础组件拼接完整文件路径 → 记忆存储组件写入文件 → 通知所有已连接的记忆结构实现组件。
...
### 大模型输出
思考内容存入思考记录。工具调用指令的 name 和 parameter 存入工具调用记录。大模型生成回复时发 channel，并由 channel 调用记忆存储。如果大模型生成文本，则构造 text 类型的 channel 消息，否则按照大模型返回的数据类型构造相应 msg_type 的消息。
