...
### 配置管理器
配置按三分结构管理：
- **AgentConfig（静态）**：从 KISSBOT_CONFIG 的 `agent` 段加载，启动后不变，含 `data_dir`、`mgmt_host`、`mgmt_port`、`ws_reconnect_interval_secs`，以及 `init_model`（首次引导 NexusRepo 默认模型）
- **NexusRepo / StationRepo（可改、落盘）**：持久化到 `<data_dir>/nexus.json` 与 `<data_dir>/station.json`，修改经 COW 写回；NexusRepo 存 channels / models / memory_structs 与 1 个 default 值（default_model），StationRepo 存 stations（本轮占位）
