...
**记忆存储模块**：收集 nexus 和消息通道的全部原始消息（包括 channel 消息、思考内容、工具调用三种类型）并持久化存储，提供记忆推送和查询 API。有新数据时通知各已连接的记忆结构模块。其中 channel 消息使用统一的公共消息格式。
...
### 5. API 定义
定义各模块间通信的标准数据结构和统一的响应格式。是模块间数据交换的契约层，确保跨模块通信的数据类型一致。
