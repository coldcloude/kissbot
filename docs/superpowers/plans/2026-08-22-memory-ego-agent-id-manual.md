...
注意：测试目录为进程级共享（test_util OnceLock<TempDir>），且 create_agent 现在有查重逻辑，各测试必须使用互不相同的 agent_id（"alice"/"alice_orig"/"alice_crud"/"Alice"/"alice_01"/"dup_alice" 均不同）；且 agent_id 须通过 validate_code（仅 [A-Za-z0-9_]，不能含连字符）。
