...
use crate::station::{TERR_TOOL_EXTERNAL_ERROR, tool_error};
use crate::types::{Error, Result, StationCallToolRequestRef, StationListMcpsRequest, StationListToolsRequest, ToolCall};
...
impl StationClient {
    ...
    pub async fn list_tools(&self, filter: Option<&HashSet<String>>, ancestors: &[String]) -> Result<Vec<Arc<ToolConfig>>> {
        let req = StationListToolsRequest {
            filter: filter.map(|set| set.iter().cloned().collect()),
            ancestors: ancestors.to_vec(),
        };
        let resp = self.post::<_, Vec<Arc<ToolConfig>>>("/station/tools", &req).await?;
        Ok(resp)
    }

    /// 查询子 Station 平铺后的 MCP 元数据（可选 toolkit 白名单过滤）
    pub async fn list_mcps(&self, filter: Option<&HashSet<String>>, ancestors: &[String]) -> Result<Vec<Arc<McpConfig>>> {
        let req = StationListMcpsRequest {
            filter: filter.map(|set| set.iter().cloned().collect()),
            ancestors: ancestors.to_vec(),
        };
        let resp = self.post::<_, Vec<Arc<McpConfig>>>("/station/mcps", &req).await?;
        Ok(resp)
    }

    /// 调用子 Station 上的工具
    pub async fn call_tool(&self, mut tool_call: ToolCall, ancestors: &[String]) -> ToolCall {
        let req = StationCallToolRequestRef {
            tool_call: &tool_call,
            ancestors,
        };
        let resp = self.post::<_, ToolCall>("/station/call-tool", &req).await;
        match resp {
            Ok(call) => call,
            // 透传失败原因：Err 文案来自发送/解析失败，或服务端 success=false/非 2xx 的错误文案，
            // 带上它才能区分「子站不可达」与「子站工具执行失败」
            Err(e) => {
                tool_call.data.error = tool_error(TERR_TOOL_EXTERNAL_ERROR, format!("调用sub station工具失败: {}", e));
                tool_call
            },
        }
    }
...
use super::*;
    ...
    async fn client_parses_call_tool_failure_as_error() {
        ...
        assert!(message.contains("调用sub station工具失败"), "统一失败文案: {}", err);
        assert!(message.contains("boom"), "应透传服务端错误文案: {}", err);
    }
