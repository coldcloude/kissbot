...
pub fn build<T>(contents: impl IntoIterator<Item = Vec<T>>, max_depth: usize) -> SingleIndex<T>
where
    T: Eq + Hash + Clone + 'static,
{
    let mut root = SingleIndex::new();
    for content in contents {
        ...
        for start in 0..content.len() {
            ...
            while let Some(token) = valid_tokens.pop_front() {
                current_tree = current_tree.sub_tree_map.get_or_insert_with(|| HashMap::new()).entry(token).or_insert_with(|| SingleIndex::new());
            }
        }
    }
    root
